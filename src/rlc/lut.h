...
#define LUT_INCLUDE(N, A, TK, MK, TV, MV) \
    ...
    N##_KV *A##_once(N *lut, const LUT_ITEM(TK, MK) key, LUT_ITEM(TV, MV) val); \
    N##_KV *A##_set(N *lut, const LUT_ITEM(TK, MK) key, LUT_ITEM(TV, MV) val); \
    TV *A##_get(N *lut, const LUT_ITEM(TK, MK) key); \
    N##_KV *A##_get_kv(N *lut, const LUT_ITEM(TK, MK) key); \
    void A##_del(N *lut, const LUT_ITEM(TK, MK) key); \
    N##_KV **A##_iter_all(N *lut, N##_KV **item); \
    int A##_dump(N *lut, N##_KV ***items, size_t *len); \
...
#define LUT_IMPLEMENT_COMMON_ONCE(N, A, TK, MK, TV, MV, H, C, FK, FV) \
    N##_KV *A##_once(N *lut, const LUT_ITEM(TK, MK) key, LUT_ITEM(TV, MV) val) { \
        ...
        size_t hash = H(key); \
        N##_KV **item = A##_static_get_item(lut, key, hash, true); \
        if(*item) { \
            ...
            *item = malloc(req); \
            ...
            if(!*item) return 0; \
            if(LUT_IS_BY_REF(MK)) { \
                N##_KV *p = (N##_KV *)((uint8_t *)*item + sizeof(**item) + 0); \
                memset(p, 0, sizeof((*item)->key)); \
                memcpy(&(*item)->key, &p, sizeof((*item)->key)); \
            } \
            if(LUT_IS_BY_REF(MV)) { \
                N##_KV *p = (N##_KV *)((uint8_t *)*item + sizeof(**item) + sizeof(*LUT_REF(MK)(*item)->key)); \
...
#define LUT_IMPLEMENT_COMMON_GET(N, A, TK, MK, TV, MV, H, C, FK, FV) \
    TV *A##_get(N *lut, const LUT_ITEM(TK, MK) key) { \
        ...
        if(!lut->width) return 0; \
        size_t hash = H(key); \
        N##_KV **item = A##_static_get_item(lut, key, hash, false); \
